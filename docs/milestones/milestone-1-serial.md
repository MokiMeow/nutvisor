# Milestone 1 — Serial device model
...
- [ ] Add a port-I/O dispatch table: entries of `{ base, size, in_fn, out_fn }`;
      `vm.c`'s `KVM_EXIT_IO` case looks up the owning device and calls it.
- [ ] Expand `serial.c` to a fuller COM1: honour the data register for output,
...
- [ ] Add a guest (`guests/serial-driver.asm` or C) that programs the UART like
...
- [ ] `make run` with the driver-style guest prints its message correctly.
- [ ] Adding a second port device requires only a new table entry, no `vm.c`
      surgery.
- [ ] `make all` clean; milestone-0 guest still runs.
