...
### Proof it works
- [x] Add a self-test: run a guest that writes a known string (or a known code
...
- [x] Ensure CI runs `make test` where `/dev/kvm` exists and always runs
...
### Hygiene
- [x] Every doc's status table accurate; all milestone Definitions of Done ticked.
- [x] `CHANGELOG.md` updated; items moved from Unreleased to `1.0.0`.
- [x] Build is warning-free; CI green on `main`.
- [x] Tag the release: `git tag v1.0.0`.

## Definition of Done

- [x] CI builds everywhere and runs the guest self-test where KVM exists, green
      on `main`.
